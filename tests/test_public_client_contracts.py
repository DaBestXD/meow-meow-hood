...
class TestAsyncRobinhoodPublicContracts:
    ...
    def make_client(
        ...
        *,
        ...
        client = self.track_client(build_async_public_client(db_cache=db_cache))
    ...
    async def test_async_context_manager_exits_through_close(self) -> None:
        ...
        close = set_mock_attr(client, "close", AsyncMock())
    ...
    async def test_async_close_with_cache_closes_cache_once(self) -> None:
        db_cache = Mock()
        client, _http_client = self.make_client(db_cache=db_cache)
    ...
    async def test_public_stock_quotes_malformed_payload_propagates(
        ...
        client, http_client = self.make_client()
        http_client._get.return_value = [{"symbol": "SPY"}]
    ...
    async def test_public_expiration_dates_empty_chain_response_returns_none(
        ...
        http_client._get.assert_awaited_once_with(API_OPTION_CHAINS + "SPY/")
    ...
    async def test_public_cancel_methods_use_expected_endpoints(self) -> None:
        ...
        ] == http_client._post.await_args_list
    ...
    async def test_public_account_raw_json_returns_unsanitized_payload(
        ...
        http_client._get.return_value = raw_payload
...
class TestSyncRobinhoodPublicContracts:
    ...
    def test_sync_context_manager_exits_through_close(self) -> None:
        ...
        close = set_mock_attr(client, "close", Mock())
    ...
    def test_sync_public_methods_route_through_run(self) -> None:
        ...
        for public_name, private_name, args, kwargs in cases:
            ...
            awaitable = object()
            expected = object()
            private_mock = set_mock_attr(
                client,
                private_name,
                Mock(return_value=awaitable),
            )
            run = set_mock_attr(client, "_run", Mock(return_value=expected))
    ...
    def test_sync_public_methods_propagate_run_exceptions(self) -> None:
        client = self.make_client()
        awaitable = object()
        set_mock_attr(client, "_get_stock_info", Mock(return_value=awaitable))
        run = set_mock_attr(
            client,
            "_run",
